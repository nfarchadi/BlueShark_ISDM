    ...
    for (i in 1:repeats){
        ...
        for (k in 1:k_folds){
            ...
            preds_bru <- predict(out.inla, data = test %>% dplyr::select(sst,sst_sd,bathy), formula = ~ plogis(sst + sst_sd + bathy + etag_field + marker_field + observer_field), 
                                n.samples = 10, num.threads = cores)
            t2 <- Sys.time()
            ...
            Evaluations_kfold_INLA[counter,14] <- i
            Evaluations_kfold_INLA[counter,15] <- "etag"
            Evaluations_kfold_INLA[counter,16] <- t3
            counter=counter+1 
            ...
            Evaluations_kfold_INLA[counter,14] <- i
            Evaluations_kfold_INLA[counter,15] <- "marker"
            Evaluations_kfold_INLA[counter,16] <- t3
            counter=counter+1 
            ...
            Evaluations_kfold_INLA[counter,14] <- i
            Evaluations_kfold_INLA[counter,15] <- "observer"
            Evaluations_kfold_INLA[counter,16] <- t3
            counter=counter+1 
            ...
            Evaluations_kfold_INLA[counter,14] <- i
            Evaluations_kfold_INLA[counter,15] <- "all"
            Evaluations_kfold_INLA[counter,16] <- t3
            counter=counter+1  
        }
    }
