# # Fitting iSDM seasonal with full dataset

INLA_seasonal_fullmodel <- function(dataInput, inla.x, inla.y, shp, cores = 1, n_samples = 1000, env_data = NULL){
    ...
    if(is.null(env_data)){
        iSDM_seasonal_ME_GMRF <- list(ME, GMRF, summary_inla, spde.range, spde.logvar)
        
        return(iSDM_seasonal_ME_GMRF)
        
    } else {
        iSDM_seasonal_ME_GMRF_preds <- list(ME, GMRF, preds_bru, summary_inla, spde.range, spde.logvar)
        
        return(iSDM_seasonal_ME_GMRF_preds)
    }
}